import type { Config } from 'tailwindcss';
import { heroui } from '@heroui/react';

export function to_rgb(hex: string): string {
  let r = 0;
  let g = 0;
  let b = 0;
  hex = hex.replace('#', '');

  if (hex.length === 3) {
    hex = hex
      .split('')
      .map((char) => char + char)
      .join('');
  }

  if (hex.length > 6) {
    hex = hex.slice(0, 6);
  }

  while (hex.length < 6) hex += '0';

  const bigint = parseInt(hex, 16);
  r = (bigint >> 16) & 255;
  g = (bigint >> 8) & 255;
  b = bigint & 255;
  return `${r}, ${g}, ${b}`;
}

export const foggy_accent = {
  light: {
    50: '#fcf5fe',
    100: '#F7E6FD',
    200: '#EDCEFC',
    300: '#DEB4F8',
    400: '#CE9FF1',
    500: '#B67FE9',
    600: '#8F5CC8',
    700: '#6B40A7',
    800: '#4A2887',
    900: '#33186F',
    foreground: '#ffffff',
    DEFAULT: '#B67FE9',
  },
  dark: {
    900: '#F7E6FD',
    800: '#EDCEFC',
    700: '#DEB4F8',
    600: '#CE9FF1',
    500: '#B67FE9',
    400: '#8F5CC8',
    300: '#6B40A7',
    200: '#4A2887',
    100: '#33186F',
    50: '#2e185d',
    foreground: '##2e185d',
    DEFAULT: '#B67FE9',
  },
  hsl_palette: {},
};
export const primary = {
  light: {
    50: '#faf9fb',
    100: '#F3F1F6',
    200: '#E9E6EE',
    300: '#D8D2E0',
    400: '#C2B7CE',
    500: '#AA99B9',
    600: '#9781A6',
    700: '#846E93',
    800: '#6F5C7B',
    900: '#5B4D65',
    foreground: '#ffffff',
    DEFAULT: '#AA99B9',
  },
  dark: {
    900: '#F3F1F6',
    800: '#E9E6EE',
    700: '#D8D2E0',
    600: '#C2B7CE',
    500: '#AA99B9',
    400: '#9781A6',
    300: '#846E93',
    200: '#6F5C7B',
    100: '#5B4D65',
    50: '#493f50',
    foreground: '#493f50',
    DEFAULT: '#AA99B9',
  },
  hsl_palette: {
    50: 'hsl(var(--heroui-primary-50))',
    100: 'hsl(var(--heroui-primary-100))',
    200: 'hsl(var(--heroui-primary-200))',
    300: 'hsl(var(--heroui-primary-300))',
    400: 'hsl(var(--heroui-primary-400))',
    500: 'hsl(var(--heroui-primary-500))',
    600: 'hsl(var(--heroui-primary-600))',
    700: 'hsl(var(--heroui-primary-700))',
    800: 'hsl(var(--heroui-primary-800))',
    900: 'hsl(var(--heroui-primary-900))',
    DEFAULT: 'hsl(var(--heroui-primary))',
    foreground: 'hsl(var(--heroui-primary-foreground))',
  },
};
export const secondary = {
  light: {
    50: '#f9f6f9',
    100: '#F4EFF3',
    200: '#EAE0E8',
    300: '#D9C8D5',
    400: '#B999B1',
    500: '#AC88A2',
    600: '#956D88',
    700: '#7D5970',
    800: '#694B5E',
    900: '#594250',
    foreground: '#FFFFFF',
    DEFAULT: '#AC88A2',
  },
  dark: {
    900: '#F4EFF3',
    800: '#EAE0E8',
    700: '#D9C8D5',
    600: '#B999B1',
    500: '#AC88A2',
    400: '#956D88',
    300: '#7D5970',
    200: '#694B5E',
    100: '#594250',
    50: '#33242d',
    foreground: '#33242d',
    DEFAULT: '#AC88A2',
  },
  hsl_palette: {
    50: 'hsl(var(--heroui-secondary-50))',
    100: 'hsl(var(--heroui-secondary-100))',
    200: 'hsl(var(--heroui-secondary-200))',
    300: 'hsl(var(--heroui-secondary-300))',
    400: 'hsl(var(--heroui-secondary-400))',
    500: 'hsl(var(--heroui-secondary-500))',
    600: 'hsl(var(--heroui-secondary-600))',
    700: 'hsl(var(--heroui-secondary-700))',
    800: 'hsl(var(--heroui-secondary-800))',
    900: 'hsl(var(--heroui-secondary-900))',
    DEFAULT: 'hsl(var(--heroui-secondary))',
    foreground: 'hsl(var(--heroui-secondary-foreground))',
  },
};
export const danger = {
  light: {
    50: '#faf7f6',
    100: '#F6ECEA',
    200: '#EFDCD9',
    300: '#E2C3BF',
    400: '#D0A099',
    500: '#B67268',
    600: '#A7655B',
    700: '#8B524A',
    800: '#744740',
    900: '#5a3a34',
    foreground: '#FFFFFF',
    DEFAULT: '#B67268',
  },
  dark: {
    900: '#F6ECEA',
    800: '#EFDCD9',
    700: '#E2C3BF',
    600: '#D0A099',
    500: '#B67268',
    400: '#A7655B',
    300: '#8B524A',
    200: '#744740',
    100: '#623F3A',
    50: '#341e1b',
    foreground: '#341e1b',
    DEFAULT: '#B67268',
  },
  hsl_palette: {
    50: 'hsl(var(--heroui-danger-50))',
    100: 'hsl(var(--heroui-danger-100))',
    200: 'hsl(var(--heroui-danger-200))',
    300: 'hsl(var(--heroui-danger-300))',
    400: 'hsl(var(--heroui-danger-400))',
    500: 'hsl(var(--heroui-danger-500))',
    600: 'hsl(var(--heroui-danger-600))',
    700: 'hsl(var(--heroui-danger-700))',
    800: 'hsl(var(--heroui-danger-800))',
    900: 'hsl(var(--heroui-danger-900))',
    DEFAULT: 'hsl(var(--heroui-danger))',
    foreground: 'hsl(var(--heroui-danger-foreground))',
  },
};
export const success = {
  light: {
    50: '#f5f8f5',
    100: '#E0EBE0',
    200: '#C3D7C4',
    300: '#99B99D',
    400: '#6F9875',
    500: '#4F7A57',
    600: '#3B6043',
    700: '#2F4D36',
    800: '#273E2D',
    900: '#213325',
    foreground: '#FFFFFF',
    DEFAULT: '#4F7A57',
  },
  dark: {
    900: '#E0EBE0',
    800: '#C3D7C4',
    700: '#99B99D',
    600: '#6F9875',
    500: '#4F7A57',
    400: '#3B6043',
    300: '#2F4D36',
    200: '#273E2D',
    100: '#213325',
    50: '#121c15',
    foreground: '#121c15',
    DEFAULT: '#4F7A57',
  },
  hsl_palette: {
    50: 'hsl(var(--heroui-success-50))',
    100: 'hsl(var(--heroui-success-100))',
    200: 'hsl(var(--heroui-success-200))',
    300: 'hsl(var(--heroui-success-300))',
    400: 'hsl(var(--heroui-success-400))',
    500: 'hsl(var(--heroui-success-500))',
    600: 'hsl(var(--heroui-success-600))',
    700: 'hsl(var(--heroui-success-700))',
    800: 'hsl(var(--heroui-success-800))',
    900: 'hsl(var(--heroui-success-900))',
    DEFAULT: 'hsl(var(--heroui-success))',
    foreground: 'hsl(var(--heroui-success-foreground))',
  },
};
export const warning = {
  light: {
    50: '#f8f7f2',
    100: '#E1DDC7',
    200: '#CEC6A3',
    300: '#EED17E',
    400: '#B8A87A',
    500: '#AB9664',
    600: '#9E8458',
    700: '#846C4A',
    800: '#6C5840',
    900: '#584936',
    foreground: '#FFFFFF',
    DEFAULT: '#AB9664',
  },
  dark: {
    900: '#E1DDC7',
    800: '#CEC6A3',
    700: '#EED17E',
    600: '#B8A87A',
    500: '#AB9664',
    400: '#9E8458',
    300: '#846C4A',
    200: '#6C5840',
    100: '#584936',
    50: '#504233',
    foreground: '#504233',
    DEFAULT: '#AB9664',
  },
  hsl_palette: {
    50: 'hsl(var(--heroui-warning-50))',
    100: 'hsl(var(--heroui-warning-100))',
    200: 'hsl(var(--heroui-warning-200))',
    300: 'hsl(var(--heroui-warning-300))',
    400: 'hsl(var(--heroui-warning-400))',
    500: 'hsl(var(--heroui-warning-500))',
    600: 'hsl(var(--heroui-warning-600))',
    700: 'hsl(var(--heroui-warning-700))',
    800: 'hsl(var(--heroui-warning-800))',
    900: 'hsl(var(--heroui-warning-900))',
    DEFAULT: 'hsl(var(--heroui-warning))',
    foreground: 'hsl(var(--heroui-warning-foreground))',
  },
};
export const info = {
  light: {
    50: '#f5f8fa',
    100: '#EAF0F5',
    200: '#D9E4EC',
    300: '#C1D2E0',
    400: '#A7BBD2',
    500: '#8EA3C2',
    600: '#798CB2',
    700: '#67779B',
    800: '#55637E',
    900: '#495366',
    foreground: '#FFFFFF',
    DEFAULT: '#8EA3C2',
  },
  dark: {
    900: '#EAF0F5',
    800: '#D9E4EC',
    700: '#C1D2E0',
    600: '#A7BBD2',
    500: '#8EA3C2',
    400: '#798CB2',
    300: '#67779B',
    200: '#55637E',
    100: '#495366',
    50: '#3b4251',
    foreground: '#3b4251',
    DEFAULT: '#8EA3C2',
  },
  hsl_palette: {
    50: 'hsl(var(--heroui-info-50))',
    100: 'hsl(var(--heroui-info-100))',
    200: 'hsl(var(--heroui-info-200))',
    300: 'hsl(var(--heroui-info-300))',
    400: 'hsl(var(--heroui-info-400))',
    500: 'hsl(var(--heroui-info-500))',
    600: 'hsl(var(--heroui-info-600))',
    700: 'hsl(var(--heroui-info-700))',
    800: 'hsl(var(--heroui-info-800))',
    900: 'hsl(var(--heroui-info-900))',
    DEFAULT: 'hsl(var(--heroui-info))',
    foreground: 'hsl(var(--heroui-info-foreground))',
  },
};

const foggy_accent_rgb = {
  light: {
    50: to_rgb(foggy_accent.light['50']),
    100: to_rgb(foggy_accent.light['100']),
    200: to_rgb(foggy_accent.light['200']),
    300: to_rgb(foggy_accent.light['300']),
    400: to_rgb(foggy_accent.light['400']),
    500: to_rgb(foggy_accent.light['500']),
    600: to_rgb(foggy_accent.light['600']),
    700: to_rgb(foggy_accent.light['700']),
    800: to_rgb(foggy_accent.light['800']),
    900: to_rgb(foggy_accent.light['900']),
    foreground: to_rgb(foggy_accent.light.foreground),
    DEFAULT: to_rgb(foggy_accent.light.DEFAULT),
  },
  dark: {
    50: to_rgb(foggy_accent.dark['50']),
    100: to_rgb(foggy_accent.dark['100']),
    200: to_rgb(foggy_accent.dark['200']),
    300: to_rgb(foggy_accent.dark['300']),
    400: to_rgb(foggy_accent.dark['400']),
    500: to_rgb(foggy_accent.dark['500']),
    600: to_rgb(foggy_accent.dark['600']),
    700: to_rgb(foggy_accent.dark['700']),
    800: to_rgb(foggy_accent.dark['800']),
    900: to_rgb(foggy_accent.dark['900']),
    foreground: to_rgb(foggy_accent.dark.foreground),
    DEFAULT: to_rgb(foggy_accent.dark.DEFAULT),
  },
};

const scrollbarPlugin = require('tailwind-scrollbar');

export default {
  content: [
    './(pages)/**/*.{js,ts,jsx,tsx,mdx}',
    './components/**/*.{js,ts,jsx,tsx,mdx}',
    './app/**/*.{js,ts,jsx,tsx,mdx}',
    '../node_modules/@heroui/theme/dist/**/*.{js,ts,jsx,tsx}',
  ],
  theme: {
    extend: {
      colors: {
        f_accent: foggy_accent.light,
        primary: primary.hsl_palette,
        secondary: secondary.hsl_palette,
        danger: danger.hsl_palette,
        success: success.hsl_palette,
        warning: warning.hsl_palette,
        info: info.hsl_palette,
      },
    },
  },
  darkMode: 'class',
  plugins: [
    heroui({
      addCommonColors: true,
      themes: {
        light: {
          colors: {
            background: '#fafafa',
            foreground: '#18181b',
            primary: primary.light,
            secondary: secondary.light,
            danger: danger.light,
            warning: warning.light,
            success: success.light,
          },
        },
        dark: {
          colors: {
            background: '#18181b',
            foreground: '#fafafa',
            primary: primary.dark,
            secondary: secondary.dark,
            danger: danger.dark,
            warning: warning.dark,
            success: success.dark,
          },
        },
      },
      layout: {
        fontSize: {
          tiny: '10px', // text-tiny
          small: '14px', // text-small
          medium: '18px', // text-medium
          large: '20px', // text-large
        },
        disabledOpacity: '0.3',
        radius: {
          small: '8px', // rounded-small
          medium: '16px', // rounded-medium
          large: '32px', // rounded-large
        },
        borderWidth: {
          small: '1px', // border-small
          medium: '1px', // border-medium
          large: '2px', // border-large
        },
      },
    }),
    scrollbarPlugin({ nocompatible: true }),
    function ({ addUtilities }: any) {
      const avatars = {
        '.accent-avatar': {
          color: foggy_accent.light.foreground,
          backgroundColor: foggy_accent.light.DEFAULT,
          boxShadow: `var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) hsl(var(--heroui-background)), var(--tw-ring-inset) 0 0 0 calc(2px + var(--tw-ring-offset-width)) ${foggy_accent.light.DEFAULT}, var(--tw-shadow, 0 0 #00000)`,
        },
      };
      const badges = {
        '.accent-badge-s': {
          borderColor: 'hsl(var(--heroui-background))',
          backgroundColor: foggy_accent.light.DEFAULT,
          color: foggy_accent.light.foreground,
        },
        '.accent-badge-fl': {
          borderColor: 'hsl(var(--heroui-background))',
          backgroundColor: `rgba(${foggy_accent_rgb.light.DEFAULT}, 0.2)`,
          color: foggy_accent.light['700'],
        },
        '.accent-badge-f': {
          color: foggy_accent.light.DEFAULT,
          borderColor: 'hsl(var(--heroui-default))',
          backgroundColor: 'hsl(var(--heroui-default-100))',
        },
        '.accent-badge-sh': {
          borderColor: 'hsl(var(--heroui-background))',
          backgroundColor: foggy_accent.light.DEFAULT,
          color: foggy_accent.light.foreground,
          boxShadow: `0 10px 15px -3px rgba(${foggy_accent_rgb.light.DEFAULT}, 0.4), 0 4px 6px -4px rgba(${foggy_accent_rgb.light.DEFAULT}, 0.4)`,
        },
      };
      const buttons = {
        /* accent-solid */
        '.accent-s': {
          backgroundColor: foggy_accent.light.DEFAULT,
          color: foggy_accent.light.foreground,
        },
        /* accent-faded */
        '.accent-f': {
          backgroundColor: 'hsl(var(--heroui-default-100))',
          border:
            'var(--heroui-border-width-medium) solid hsl(var(--heroui-default))',
          color: foggy_accent.light.DEFAULT,
        },
        /* accent-bordered */
        '.accent-b': {
          backgroundColor: 'rgba(0, 0, 0, 0)',
          border: `var(--heroui-border-width-medium) solid ${foggy_accent.light.DEFAULT}`,
          color: foggy_accent.light.DEFAULT,
        },
        /* accent-light */
        '.accent-l': {
          backgroundColor: 'rgba(0, 0, 0, 0)',
          color: foggy_accent.light.DEFAULT,
        },
        '.accent-l-hover': {
          opacity: '1',
          backgroundColor: `rgba(${foggy_accent_rgb.light.DEFAULT}, 0.25)`,
        },
        /* accent-flat */
        '.accent-fl': {
          backgroundColor: `rgba(${foggy_accent_rgb.light.DEFAULT}, 0.2)`,
          color: foggy_accent.light.DEFAULT,
        },
        /* accent-ghost */
        '.accent-g': {
          backgroundColor: 'rgba(0, 0, 0, 0)',
          border: `var(--heroui-border-width-medium) solid ${foggy_accent.light.DEFAULT}`,
          color: foggy_accent.light.DEFAULT,
        },
        '.accent-g-hover': {
          backgroundColor: foggy_accent.light.DEFAULT,
          color: foggy_accent.light.foreground,
        },
        /* accent-shadow */
        '.accent-sh': {
          backgroundColor: foggy_accent.light.DEFAULT,
          color: foggy_accent.light.foreground,
          boxShadow: `0 10px 15px -3px rgba(${foggy_accent_rgb.light.DEFAULT}, 0.4), 0 4px 6px -4px rgba(${foggy_accent_rgb.light.DEFAULT}, 0.4)`,
        },
      };

      const theme_styles = {
        '.shadow-container': {
          boxShadow: `2px 4px 10px 0 hsl(var(--heroui-primary-500)/0.06)`,
        },
        '.dark .shadow-container': {
          boxShadow: `1px 4px 10px 0 rgba(${foggy_accent_rgb.dark['100']}, 0.02)`,
        },
        '.accent-link': {
          transitionProperty: 'color, text-decoration-color, fill, stroke',
          transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)',
          transitionDuration: '200ms',
        },
        '.accent-link-hover': {
          color: `rgb(${to_rgb(foggy_accent.light.DEFAULT)})`,
        },
      };

      addUtilities({ ...avatars, ...badges, ...buttons, ...theme_styles }, [
        'responsive',
        'hover',
      ]);
    },
  ],
} satisfies Config;
